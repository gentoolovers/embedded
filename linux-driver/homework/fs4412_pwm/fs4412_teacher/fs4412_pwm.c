...
static int pwm_major = 251;
static int pwm_minor = 0;
static int number_of_device =1;
struct fs4412_pwm_dev
{
    unsigned int *gpdcon;
    void __iomem *timer_base;
    struct cdev cdev;
};
static struct fs4412_pwm_dev *pwm;
...
static int __init fs4412_pwm_init(void)
{
    ...
    ret = register_chrdev_region(devno,number_of_device,"fs4412_pwm");
    if(ret <0)
    {
        printk("failpwm to register_chrdev_region\n");
        return ret;
    }
